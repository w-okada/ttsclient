...
def predict(session, onnx_input: Dict[str, Any], labels: List[str]) -> Tuple[List[str], List[float]]:
    all_preds = []
    all_confidences = []
    probs = session.run([], {"input_ids": onnx_input["input_ids"], "token_type_ids": onnx_input["token_type_ids"], "attention_mask": onnx_input["attention_masks"], "phoneme_mask": onnx_input["phoneme_masks"], "char_ids": onnx_input["char_ids"], "position_ids": onnx_input["position_ids"]})[0]
...
def download_and_decompress(model_dir: str = "G2PWModel/"):
    if not os.path.exists(model_dir):
        ...
        response = requests.head(modelscope_url)
        ...
        with requests.get(modelscope_url, stream=True) as r:
            ...
            with open(zip_dir, "wb") as f:
                for chunk in r.iter_content(chunk_size=8192):
                    dowload_chunks += 1
                    if dowload_chunks % 100 == 0:
                        print("downloading...", file_size, dowload_chunks * 8192)
                    if chunk:
                        f.write(chunk)
...
class G2PWOnnxConverter:
    def __init__(self, model_dir: str = "G2PWModel/", style: str = "bopomofo", model_source: str = None, enable_non_tradional_chinese: bool = False):
        ...
        self.non_polyphonic = {"一", "不", "和", "咋", "嗲", "剖", "差", "攢", "倒", "難", "奔", "勁", "拗", "肖", "瘙", "誒", "泊", "听", "噢"}
        ...
        self.labels, self.char2phonemes = get_char_phoneme_labels(polyphonic_chars=self.polyphonic_chars) if self.config.use_char_phoneme else get_phoneme_labels(polyphonic_chars=self.polyphonic_chars)
    ...
    def __call__(self, sentences: List[str]) -> List[List[str]]:
        ...
        onnx_input = prepare_onnx_input(tokenizer=self.tokenizer, labels=self.labels, char2phonemes=self.char2phonemes, chars=self.chars, texts=texts, query_ids=query_ids, use_mask=self.config.use_mask, window_size=None)
