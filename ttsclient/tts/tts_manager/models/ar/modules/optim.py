...
class ScaledAdam(BatchedOptimizer):
    ...
    def __init__(
        ...
        assert parameters_names is not None, "Please prepare parameters_names," "which is a List[List[str]]. Each List[str] is for a group" "and each str is for a parameter"
        defaults = dict(
            lr=lr,
    ...
    def _get_clipping_scale(self, group: dict, tuples: List[Tuple[Tensor, dict, List[str]]]) -> float:
        ...
        if step % clipping_update_period == 0:
            ...
            percent_clipped = first_state["num_clipped"] * 100.0 / clipping_update_period if "num_clipped" in first_state else 0.0
            first_state["num_clipped"] = 0
            quartiles = " ".join(["%.3e" % x for x in quartiles])
            logging.info(f"Clipping_scale={clipping_scale}, grad-norm quartiles {quartiles}, " f"threshold={threshold:.3e}, percent-clipped={percent_clipped:.1f}")
        ...
        if step < clipping_update_period:
            ...
            try:
                ...
                logging.info("Warning: model_norm_threshold not in state: possibly " "you changed config when restarting, adding clipping_scale option?")
                return 1.0
    ...
    def _show_gradient_dominating_parameter(self, tuples: List[Tuple[Tensor, dict, List[str]]], tot_sumsq: Tensor):
        ...
        for p, state, batch_param_names in tuples:
            ...
            for name, sumsq_orig, rms, grad in zip(batch_param_names, batch_sumsq_orig, batch_rms_orig, batch_grad):
        ...
        logging.info(f"Parameter Dominanting tot_sumsq {dominant_param_name}" f" with proportion {dominant_proportion:.2f}," f" where dominant_sumsq=(grad_sumsq*orig_rms_sq)" f"={dominant_sumsq:.3e}," f" grad_sumsq = {(dominant_grad**2).sum():.3e}," f" orig_rms_sq={(dominant_rms**2).item():.3e}")
    ...
    def _size_update(self, group: dict, scale_grads: Tensor, p: Tensor, state: dict) -> None:
        """
