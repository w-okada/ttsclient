...
from ttsclient.tts.tts_manager.models.ar.onnx.embedding_onnx import SinePositionalEmbedding, TokenEmbedding
from ttsclient.tts.tts_manager.models.ar.onnx.transformer_onnx import LayerNorm, TransformerEncoder, TransformerEncoderLayer
...
class T2SFirstStageDecoder(nn.Module):
    ...
    def forward(self, x, prompt):
        ...
        y_attn_mask = torch.cumsum(y_attn_mask, dim=1) - torch.cumsum(torch.ones_like(y_example.transpose(0, 1), dtype=torch.int64), dim=0)
        ...
        cache["k"] = torch.matmul(x_attn_mask_pad[0].float().unsqueeze(-1), torch.zeros((1, 512))).unsqueeze(1).repeat(self.num_layers, 1, 1, 1)
        cache["v"] = torch.matmul(x_attn_mask_pad[0].float().unsqueeze(-1), torch.zeros((1, 512))).unsqueeze(1).repeat(self.num_layers, 1, 1, 1)
...
class T2SStageDecoder(nn.Module):
    ...
    def forward(self, y, k, v, y_emb, x_example):
        ...
        # samples = sample(logits[0], y, top_k=self.top_k, top_p=1.0, repetition_penalty=1.35)[0].unsqueeze(0)
        samples = sample(logits[0], y, top_k=self.top_k, top_p=1.0)[0].unsqueeze(0)
...
class Text2SemanticDecoder(nn.Module):
    ...
    def init_onnx(self):
        ...
        self.first_stage_decoder = T2SFirstStageDecoder(self.ar_audio_embedding, self.ar_audio_position, self.h, self.ar_predict_layer, self.loss_fct, self.ar_accuracy_metric, self.top_k, self.early_stop_num, self.num_layers)
        self.stage_decoder = T2SStageDecoder(self.ar_audio_embedding, self.ar_audio_position, self.h, self.ar_predict_layer, self.loss_fct, self.ar_accuracy_metric, self.top_k, self.early_stop_num, self.num_layers)
