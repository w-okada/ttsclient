...
from ttsclient.tts.tts_manager.embedder.models.cnhubert import CNHubert
...
class TTS_Config:
    ...
    def __init__(self, configs: Union[dict, str] | None = None):
        ...
        if isinstance(configs, str):
            ...
            configs = self._load_configs(self.configs_path)
...
class TTS:
    def __init__(self, configs: Union[dict, str, TTS_Config]):
        if isinstance(configs, TTS_Config):
            self.configs: TTS_Config = configs
        else:
            self.configs = TTS_Config(configs)

        self.t2s_model: Text2SemanticLightningModule | None = None
        self.vits_model: SynthesizerTrn | None = None
        self.bert_tokenizer: AutoTokenizer = None
        self.bert_model: AutoModelForMaskedLM = None
        self.cnhuhbert_model: CNHubert | None = None
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        kwargs = hps["model"]
        vits_model = SynthesizerTrn(self.configs.filter_length // 2 + 1, self.configs.segment_size // self.configs.hop_length, n_speakers=self.configs.n_speakers, **kwargs)
    ...
    def _set_prompt_semantic(self, ref_wav_path: str):
        ...
        with torch.no_grad():
            ...
            wav16k = torch.from_numpy(wav16k)  # type: ignore
            zero_wav_torch = torch.from_numpy(zero_wav)
            wav16k = wav16k.to(self.configs.device)  # type: ignore
            zero_wav_torch = zero_wav_torch.to(self.configs.device)
            if self.configs.is_half:
                wav16k = wav16k.half()  # type: ignore
                zero_wav_torch = zero_wav_torch.half()

            wav16k = torch.cat([wav16k, zero_wav_torch])  # type: ignore
            assert isinstance(self.cnhuhbert_model, CNHubert)
            hubert_feature = self.cnhuhbert_model.model(wav16k.unsqueeze(0))["last_hidden_state"].transpose(1, 2)  # type: ignore
            codes = self.vits_model.extract_latent(hubert_feature)  # type: ignore

            prompt_semantic = codes[0, 0].to(self.configs.device)
            self.prompt_cache["prompt_semantic"] = prompt_semantic

    # def batch_sequences(self, sequences: List[torch.Tensor], axis: int = 0, pad_value: int = 0, max_length: int | None = None):
    #     seq = sequences[0]
    #     ndim = seq.dim()
    #     if axis < 0:
    #         axis += ndim
    #     dtype: torch.dtype = seq.dtype
    #     pad_value = torch.tensor(pad_value, dtype=dtype)  # type: ignore
    ...
    def to_batch(
        ...
        if split_bucket:
            ...
            index_and_len_list = np.array(index_and_len_list, dtype=np.int64)  # type: ignore

            batch_index_list_len = 0
            pos = 0
            while pos < index_and_len_list.shape[0]:  # type: ignore
                # batch_index_list.append(index_and_len_list[pos:min(pos+batch_size,len(index_and_len_list))])
                pos_end = min(pos + batch_size, index_and_len_list.shape[0])  # type: ignore
                while pos < pos_end:
                    batch = index_and_len_list[pos:pos_end, 1].astype(np.float32)  # type: ignore
                    score = batch[(pos_end - pos) // 2] / (batch.mean() + 1e-8)
                    if (score >= threshold) or (pos_end - pos == 1):
                        batch_index = index_and_len_list[pos:pos_end, 0].tolist()  # type: ignore
    ...
    def run(self, inputs: dict):
        ...
        actual_seed = set_seed(seed)  # noqa
        parallel_infer = inputs.get("parallel_infer", True)
        repetition_penalty = inputs.get("repetition_penalty", 1.35)

        if parallel_infer:
            print("并行推理模式已开启")
            assert isinstance(self.t2s_model, Text2SemanticLightningModule)
        ...
        if not return_fragment:
            ...
            data, batch_index_list = self.to_batch(data, prompt_data=self.prompt_cache if not no_prompt_text else None, batch_size=batch_size, threshold=batch_threshold, split_bucket=split_bucket, device=self.configs.device, precision=self.precision)
        else:
            print("############ 切分文本 ############")
            ...
            def make_batch(batch_texts):
                ...
                if len(batch_data) == 0:
                    return None
                batch, _ = self.to_batch(batch_data, prompt_data=self.prompt_cache if not no_prompt_text else None, batch_size=batch_size, threshold=batch_threshold, split_bucket=False, device=self.configs.device, precision=self.precision)
                return batch[0]
        ...
        try:
            ...
            for item in data:  # type: ignore
                ...
                batch_phones_len: torch.LongTensor = item["phones_len"]  # noqa
                ...
                pred_semantic_list, idx_list = self.t2s_model.model.infer_panel(
                    ...
                    early_stop_num=self.configs.hz * self.configs.max_sec,  # type: ignore
                    max_len=max_len,
                ...
                refer_audio_spec: torch.Tensor = [item.to(dtype=self.precision, device=self.configs.device) for item in self.prompt_cache["refer_spec"]]  # type: ignore
                ...
                if speed_factor == 1.0:
                    # ## vits并行推理 method 2
                    pred_semantic_list = [item[-idx:] for item, idx in zip(pred_semantic_list, idx_list)]
                    assert isinstance(self.vits_model, SynthesizerTrn)
                    ...
                    _batch_phones = torch.cat(batch_phones).unsqueeze(0).to(self.configs.device)  # type: ignore
                    ...
                    for i, idx in enumerate(idx_list):
                        phones = batch_phones[i].unsqueeze(0).to(self.configs.device)  # type: ignore
                ...
                if return_fragment:
                    ...
                    yield self.audio_postprocess([batch_audio_fragment], self.configs.sampling_rate, None, speed_factor, False, fragment_interval)  # type: ignore
                else:
            ...
            if not return_fragment:
                ...
                if len(audio) == 0:
                    yield self.configs.sampling_rate, np.zeros(int(self.configs.sampling_rate), dtype=np.int16)
                    return
                yield self.audio_postprocess(audio, self.configs.sampling_rate, batch_index_list, speed_factor, split_bucket, fragment_interval)  # type: ignore
    ...
    def audio_postprocess(self, audio: List[torch.Tensor], sr: int, batch_index_list: list = None, speed_factor: float = 1.0, split_bucket: bool = True, fragment_interval: float = 0.3) -> Tuple[int, np.ndarray]:  # type: ignore
        ...
        for i, batch in enumerate(audio):
            for j, audio_fragment in enumerate(batch):
                ...
                audio_fragment: torch.Tensor = torch.cat([audio_fragment, zero_wav], dim=0)  # type: ignore
        ...
        if split_bucket:
            ...
            audio = sum(audio, [])  # type: ignore

        audio = np.concatenate(audio, 0)  # type: ignore
        audio = (audio * 32768).astype(np.int16)  # type: ignore
