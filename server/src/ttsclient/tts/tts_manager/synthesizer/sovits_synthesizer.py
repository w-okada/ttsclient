...
class SovitsSynthesizer(Synthesizer):
    def __init__(self, model_path: Path, device_id: int, use_onnx: bool, del_enc: bool = True):
        ...
        if self.use_onnx is False:
            ...
            self.vq_model.flow.remove_weight_norm()
    ...
    @property
    def is_v2pro(self):
        return hasattr(self.vq_model, 'is_v2pro') and self.vq_model.is_v2pro
    ...
    def generate_onnx_sovits(cls, dict_s2, onnx_path: Path):
        import onnx
        from onnxsim import simplify
        from ttsclient.tts.tts_manager.models.synthesizer.onnx.models_onnx import SynthesizerTrn as SynthesizerTrnOnnx
    ...
    def generate_onnx_spec(cls, dict_s2, onnx_path: Path):
        import onnx
        from onnxsim import simplify
        from ttsclient.tts.tts_manager.models.synthesizer.onnx.models_onnx import Spectrogram
    ...
    def generate_onnx_latent(cls, dict_s2, onnx_path: Path):
        import onnx
        from onnxsim import simplify
        from ttsclient.tts.tts_manager.models.synthesizer.onnx.models_onnx import SynthesizerTrnLatent
